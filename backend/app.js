...
Parse.initialize("G1o3jtJRe1HTEPIMwtwtUFNVwR0WaB40EkaB1a5v",   "UWmig8troZyoOxr0v2U8eTDFdtLNzzG96UpNwgBu");
...
app.post('/cases', (req, res) => {  //create case
    ...
    myNewObject.set('file', new Parse.File("resume.txt", { base64: req.body.base64 }));
    myNewObject.set('belongTo', { __type: "Pointer",  _objCount: 0, className: '_User', objectId:  req.body._id });
    console.log(req.body._id);
    myNewObject.set('order', Number(req.body.number));

    myNewObject.save().then(
    (result) => {
        if (typeof document !== 'undefined') document.write(`case created: ${JSON.stringify(result)}`);
        res.send(result);
        console.log('case created', result);
    },
    (error) => {
        if (typeof document !== 'undefined') document.write(`Error while creating case: ${JSON.stringify(error)}`);
